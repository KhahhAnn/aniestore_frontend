import { api, apiPermitAll } from "../../config/ApiConfig";
...
export const findProducts = (reqData) => async (dispatch) => {
   ...
   try {
      const { data } = await apiPermitAll.get(`/api/products?color=${colors}&size=${sizes}&minPrice=${minPrice}&maxPrice=${maxPrice}&minDiscount=${minDiscount}&category=${category}&stock=${stock}&sort=${sort}&pageNumber=${pageNumber}&pageSize=${pageSize}`)
...
export const findProductById = (reqData) => async (dispatch) => {
   ...
   try {
      const { data } = await apiPermitAll.get(`/api/products/${productId}`)
