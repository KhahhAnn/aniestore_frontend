...
export default function Product() {
   ...
   const [filteredProductsPage, setFilteredProductsPage] = useState();
   ...
   const handleSearchChange = (e) => {
      ...
      setFilteredProducts(productStore.products?.content?.filter(product =>
   ...
   useEffect(() => {
      ...
      const data = {
         category: param.levelThree ? param.levelThree : "all",
         colors: colorValue || [],
         sizes: sizeValue || [],
         minPrice,
         maxPrice,
      ...
      setIsLoading(false);
      setFilteredProducts(productStore.products.content)
                                    ...
                                    {({ open }) => (
                                       <>
                                          <h3 className="my-3 flow-root w-[13rem]">
                                             <Disclosure.Button className="flex items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500  w-[13rem]">
                                                <span className="font-medium text-gray-900">{section.name}</span>
                                                <span className="ml-6 flex items-center">
                                                   {open ? (
                                                      <MinusIcon className="h-5 w-5" aria-hidden="true" />
                        ...
                        <div className="lg:col-span-4 w-[95%]">
                           <div >
                              <div className='grid grid-cols-3 gap-10 bg-white py-5'>
                                 ...
                                 ) : (!searchTerm && productStore.products.content && productStore.products.content.length > 0 ? (
                                    productStore.products.content.map((item) => <ProductCard key={item.id} product={item} />)
                                 ) : (
                                    <div className="col-span-3 text-center text-gray-500">Không có sản phẩm nào</div>
                                 )))}
                              </div>
